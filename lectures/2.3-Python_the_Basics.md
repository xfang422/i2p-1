...
| Sum | `x + y` | `”HelloYou”` |
| Difference | `x - y` | `5` |
| Product | `x * z` | `HelloHello` | Equality | `x==y` | `True` or `False` |
...
The string (`str`) `"42"` is *not* the same as the integer (`int`) `42`. Here's the output from some attempts at comparison:
```python
x='42'
y=42
x==y   # False
